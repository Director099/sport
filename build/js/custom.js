  ...
  */
  ...
  function ValidateForm(elem) {
    ...
    this.addInvalid = function (elem) {
      var parentElem = $(elem).closest('label');
    ...
    this.isValid = function () {
      const typeRadio = _this.elem.find('[data-required=1][type="radio"]');
      ...
      if (!typeRadio.is(':checked')) {
        typeRadio.each(function (i, elem) {
          _this.addInvalid(elem);
        });
      }
  ...
  $("[data-date]");
  const inputDataDate = $("#js-input-data");
  btnFilter.on("click", function () {
    ...
    if (inputDataDate) {
      inputDataDate.val($(this).data('date'));
      inputDataDate.closest('.js-form').find('[type="radio"]').prop('checked', false);
    }
